...
(function (ServerConfigNames) {
    ServerConfigNames["SERVER_PROTOCOL"] = "protocol";
    ServerConfigNames["SERVER_NO_SSL"] = "noSSL";
    ServerConfigNames["SERVER_CA_FILE"] = "sslCAFile";
    ServerConfigNames["SERVER_CERT_FILE"] = "sslCertFile";
    ServerConfigNames["SERVER_KEY_FILE"] = "sslKeyFile";
})(ServerConfigNames = exports.ServerConfigNames || (exports.ServerConfigNames = {}));
...
class ServerAddInImpl extends index_1.BasicAddIn {
    ...
    addIn(server, addIns) {
        ...
        server.start = (listener) => {
            ...
            const port = config.get(index_1.CommonConfigNames.PORT);
            if (!(config.get(ServerConfigNames.SERVER_NO_SSL)) && config.get(ServerConfigNames.SERVER_PROTOCOL) === 'https') {
                ...
                try {
                    let options = {
                        key: fs_1.readFileSync(config.get(ServerConfigNames.SERVER_KEY_FILE)),
                        cert: fs_1.readFileSync(config.get(ServerConfigNames.SERVER_CERT_FILE)),
                        ca: fs_1.readFileSync(config.get(ServerConfigNames.SERVER_CA_FILE))
                    };
